...
│   └── Settings/    SettingsView (root), ProviderSettingsView (+ RegionPickerView), SharingSettingsView, LegacyImportView
├── Services/
...
- **Sync status is observable**: `PersistenceController.lastSyncEvents` (latest setup/import/export event, from `eventChangedNotification`), `isSyncing`, `lastSyncError` (human text + domain/code + first partial-error item), `lastSuccessfulSync`, `syncStatusSummary`. Settings → About shows it (with the error text, selectable) so a TestFlight build can explain itself; the Sharing screen's unshared card warns while an export is in flight or after a failed one. `createShare()` races `container.share` against a 60 s timeout (`PersistenceError.shareTimedOut`) — the share sheet otherwise spins forever behind a first-launch export to a fresh CloudKit environment.
