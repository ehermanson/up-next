...
- **Share links on cold launch** arrive in `UIScene.ConnectionOptions.cloudKitShareMetadata` (`SceneDelegate.scene(_:willConnectTo:options:)`), not through `userDidAcceptCloudKitShareWith` — both paths call the same accept routine.
