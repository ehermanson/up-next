...
- **Trap**: a `NavigationSplitView` *sidebar column* reports `.compact` to its contents. Only the view that owns the split (the tab view / `MyListsView`) may read the size class; anything inside the sidebar (`MediaListView`, `CustomListDetailView`) gets explicit flags (`selectionIsSticky` / `highlightsSelection` / `pinsSelection`). Reading the environment there once caused the Collections sheet to open alongside the pinned column.
- Sidebar columns get `.toolbar(removing: .sidebarToggle)` and `.containerBackground(for: .navigation) { AppBackground() }` (the view-builder overload — the `ShapeStyle` overload doesn't take a `View`); detail columns get the same container background. Sticky selection: a re-tap re-sends the id, only removal clears it; the pinned row is outlined with an accent `strokeBorder` at `DesignTokens.Radius.card`.
- Pinned `MediaDetailView` hides its navigation bar entirely and moves Add/Remove into the glass `actionButtonRow` (nothing to align with the sidebar's bar).
